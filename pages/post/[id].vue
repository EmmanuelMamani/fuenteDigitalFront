...
<template>
    <div>
        <div v-if="post" class="bg-white w-11/12 md:w-4/5 mx-auto rounded shadow-lg p-6 space-y-3 mt-3">
            <h1 class="text-center text-lg md:text-3xl text-slate-800 font-semibold">{{ post.title }}</h1>

            <div class="flex items-center space-x-2 text-sm md:text-base text-sky-700">
                <Icon name="ic:outline-access-time" />
                <NuxtTime :datetime="post.created_at" relative />
            </div>

            <el-carousel class="max-h-[170px] md:max-h-[300px]">
            ...
            <div v-html="post.description" class=" text-sm md:text-lg text-justify text-slate-600 md:leading-9"></div>

            <h2 class="text-center text-base md:text-2xl text-slate-800 font-semibold">Quizas te interese tambien</h2>
            <div class="grid grid-cols-2 md:grid-cols-3 w-full gap-4 mt-2 mx-auto">
