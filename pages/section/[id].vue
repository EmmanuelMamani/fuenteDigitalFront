<template>
    <div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 w-11/12 md:w-4/5 gap-4 mt-2 mx-auto" v-if="posts.length > 1">
...
<script setup>
    ...
    const idSection= route.params.id
    const posts=ref([])
    ...
    async function postList() {
        try {
            const response = await $fetch(`${config.public.apiBase}/section/${idSection}/posts?page=${actualPage.value}`)
